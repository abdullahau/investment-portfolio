...
class Portfolio:
    ...
    def _cumulative_split_factors(self, split_series: pd.Series) -> pd.Series:
        ...
        cumulative = factors.iloc[::-1].cumprod().iloc[::-1]
        return cumulative.ffill().fillna(1.0)
    ...
    def calculate_holdings_and_value(self):
        """Main calculation engine to run all portfolio processing steps."""
        self._prepare_trade_log()
        self._fetch_price_data()
        self._convert_prices_to_base_currency()  # Must run before other calcs
        self._calculate_income()

        print("Calculating daily holdings...")
        for symbol in self.symbols:
            trades = self.holdings["trade"].get(
                symbol, pd.Series(0.0, index=self.date_range)
            )
            splits = (
                self.holdings["raw_splits"]
                .get(symbol, pd.Series(0.0, index=self.date_range))
                .replace(0, 1)
            )

            holdings_col = self.holdings["holding"][symbol]

            holdings_col = trades.cumsum()
