...
py_file: Path = Path(sys.argv[1])
qmd_file: Path = Path(sys.argv[2])
...
snippet_pattern: Pattern[str] = re.compile(
...
# --- read qmd file ---
qmd_text: str = qmd_file.read_text()

# --- find code fences with insert option ---
fence_pattern: Pattern[str] = re.compile(
    r"```{python\s+insert:([\w-]+)}\n(.*?)```", re.DOTALL
)
...
def replace_code(match: Match[str]) -> str:
    ...
    code: str = snippets.get(name, f"# [ERROR: snippet '{name}' not found]")
    return f"```{{python insert:{name}}}\n{code}\n```"


# --- replace and write out ---
new_qmd: str = fence_pattern.sub(replace_code, qmd_text)

# --- backup and write ---
backup_path: Path = qmd_file.with_suffix(".bak.qmd")
